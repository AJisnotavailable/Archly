...
flowchart TB
    subgraph Client ["Client Layer (Desktop / Web)"]
        UI["React 18 + Vite 5 UI (Obsidian Glass Theme)"]
        Router["View Router (Studio, History, Lab, Settings)"]
        Viewer["MarkdownViewer + Mermaid Diagram Engine"]
        Tauri["Tauri 2.0 Rust Runtime (Process Daemon & Native Dialogs)"]
    end

    subgraph Server ["Backend API Layer (Node.js / Express :4000)"]
        API["Express Router (/api/projects, /api/providers)"]
        SSE["Server-Sent Events (SSE) Streaming Hub"]
        Orchestrator["Pipeline Orchestrator (State Machine & Pacing)"]
        CompEngine["GZIP Compression Middleware"]
    end

    subgraph Adapters ["AI Model Adapter Layer"]
        Groq["Groq Cloud Adapter (LPU / Llama 3.3 70B)"]
        Nvidia["NVIDIA NIM Adapter (Nemotron & Enterprise Models)"]
        OpenRouter["OpenRouter Cloud Gateway"]
        LocalLLM["Local LLM Adapter (Ollama / vLLM / LM Studio)"]
        DemoProv["Demo Provider (Deterministic Offline Simulation)"]
    end

    subgraph Generators ["Artifact Compilation Engine"]
        PDF["Vector PDF Engine (PDFKit Custom Obsidian Theme)"]
        HTML["Standalone Printable HTML Generator"]
        ZIP["ZIP Packager (Archiver Level 6)"]
    end

    subgraph Database ["Data & Storage Layer"]
        Prisma["Prisma ORM Client"]
        SQLite[("SQLite dev.db (Indexed)")]
        Disk[("Local Storage /storage/projects/:id")]
    end

    UI -->|HTTP / REST| API
    UI -->|EventSource SSE| SSE
    Tauri -->|Spawns & Monitors| Server
    API --> Orchestrator
    Orchestrator -->|Streams Tokens| SSE
